...
export default function HistoryPage() {
  ...
  const loadUsers = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await AdminService.getUsersWithHistory(authState.accessToken);
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredUserLogs.map((user) => (
                          <tr key={user.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className="text-sm text-gray-900">{AdminUtils.formatSessionDuration((user.loginStats?.totalLoginHours || 0) * 60)}</span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="space-y-6">
            <div className="bg-white border rounded p-8 text-center">
              ...
              <p className="text-gray-600">
                This feature will track system changes and user activities.
                <br />
                Coming soon with detailed audit trails.
              </p>
            </div>
          </div>
        )}
