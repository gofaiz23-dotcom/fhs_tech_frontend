        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {marketplaces.map((m) => (
            <div key={m.name} className="border rounded bg-white p-6 flex flex-col items-center text-center gap-3">
              <ChannelLogo name={m.name} />
              <div className="font-semibold text-gray-800">{m.name}</div>
              <div className="text-xs text-gray-600 max-w-xs">{m.desc}</div>
              <Link href={`/settings/channels/${m.name.split(' ')[0].toLowerCase()}`} className="mt-2 bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded">Connect</Link>
            </div>
          ))}
        </div>
      </div>
