...
export default function SideNav() {
  ...
  return (
    <>
      <button className="md:hidden border rounded px-2 py-1 text-sm bg-indigo-600/90 text-white" onClick={()=>setMobileOpen(!mobileOpen)}>
        {mobileOpen ? <X size={18} /> : <Menu size={18} />}
      </button>
      <aside className={`bg-indigo-600/90 text-white rounded h-fit ${collapsed ? 'w-14' : 'w-64'} transition-all duration-300 ${mobileOpen ? 'block' : 'hidden'} md:block`}> 
        <div className="p-2 flex items-center justify-between">
          <div className="text-xs font-semibold text-white/80">{!collapsed ? 'ACCOUNT SETTINGS' : 'AS'}</div>
          <button className="text-xs text-white/80" onClick={() => setCollapsed(!collapsed)}>{collapsed ? '<ChevronsRight />' : '<ChevronsLeft />'}</button>
        </div>
        <div className="px-2 space-y-1">
          ...
          {item('/settings/manage-users','Manage Users', (p:any)=> <Users {...p} color="#60a5fa" />)}
